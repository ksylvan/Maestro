/**
 * useLongPress - Generic long-press gesture hook for touch interactions.
 *
 ...
 * On the desktop renderer running on phones (web-desktop build) this is the
 * canonical way to expose right-click-only affordances (context menus, tab
 * action overlays) to touch users.
 *
 * Hoisted from `src/web/hooks/useLongPress.ts`; imports the shared touch
 * primitives from `src/renderer/utils/touch.ts`.
 */
