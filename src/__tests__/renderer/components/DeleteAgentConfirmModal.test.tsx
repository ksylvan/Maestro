...
describe('DeleteAgentConfirmModal', () => {
	...
	describe('keyboard interaction', () => {
		it('calls onClose and stops propagation when Enter is pressed on Cancel', () => {
			const parentHandler = vi.fn();
			const onClose = vi.fn();

			renderWithClosingParent({
				parentHandler,
				onClose,
				onConfirm: vi.fn(),
				onConfirmAndErase: vi.fn(),
		...
		it('calls onConfirm then onClose and stops propagation when Enter is pressed on Agent Only', () => {
			const callOrder: string[] = [];
			const parentHandler = vi.fn();
			const onClose = vi.fn(() => callOrder.push('close'));
			const onConfirm = vi.fn(() => callOrder.push('confirm'));
			...
			fireEvent.keyDown(screen.getByRole('button', { name: 'Agent Only' }), { key: 'Enter' });
		...
		it('calls onConfirmAndErase then onClose and stops propagation when Enter is pressed after typing the agent name', () => {
			const callOrder: string[] = [];
			const parentHandler = vi.fn();
			const onClose = vi.fn(() => callOrder.push('close'));
			const onConfirm = vi.fn();
			...
			fireEvent.change(screen.getByRole('textbox', { name: /confirm agent name/i }), {
				target: { value: 'TestAgent' },
			});
			fireEvent.keyDown(screen.getByRole('button', { name: 'Agent + Working Directory' }), {
				key: 'Enter',
			});
		...
		it('ignores non-Enter keys on Agent Only', () => {
			const onClose = vi.fn();
			const onConfirm = vi.fn();
			const onConfirmAndErase = vi.fn();
			...
			fireEvent.keyDown(screen.getByRole('button', { name: 'Agent Only' }), { key: 'a' });
