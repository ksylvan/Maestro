...
describe('useActivityTracker', () => {
	...
	describe('cleanup on unmount', () => {
		...
		it('ignores a stale interval tick after cleanup has cleared the interval ref', () => {
			...
			act(() => {
				vi.advanceTimersByTime(ACTIVITY_TIMEOUT_MS + TICK_INTERVAL_MS);
				tick();
			});
	...
	describe('visibility changes', () => {
		...
		it('stops tracking while hidden and restarts when an active document becomes visible', () => {
			const setIntervalSpy = vi.spyOn(global, 'setInterval');
			const clearIntervalSpy = vi.spyOn(global, 'clearInterval');
			const { result } = renderHook(() => useActivityTracker('session-1', mockSetSessions));
			...
			const startedIntervals = setIntervalSpy.mock.calls.length;
			expect(startedIntervals).toBeGreaterThan(0);
			...
			expect(setIntervalSpy.mock.calls.length).toBeGreaterThan(startedIntervals);
		});

		it('does not start an interval when activity happens while the document is hidden', () => {
			const setIntervalSpy = vi.spyOn(global, 'setInterval');
			setDocumentHidden(true);
		...
		it('does not restart tracking when an inactive document becomes visible', () => {
			const setIntervalSpy = vi.spyOn(global, 'setInterval');
			renderHook(() => useActivityTracker('session-1', mockSetSessions));
