...
describe('resolveSessionFromPidWalk', () => {
	it('returns the session id when the start pid is directly known', () => {
		const lookup = vi.fn((pid: number) => (pid === 100 ? 'sess-A' : null));
		const getParent = vi.fn(() => null);

		expect(resolveSessionFromPidWalk(100, lookup, getParent)).toBe('sess-A');
		expect(lookup).toHaveBeenCalledTimes(1);
		// We hit on first try, so the walk does not consult getParent.
		expect(getParent).not.toHaveBeenCalled();
	});

	it('walks up the parent chain until it finds a known ancestor', () => {
		...
		const getParent = vi.fn((pid: number) => parents[pid] ?? null);

		expect(resolveSessionFromPidWalk(500, lookup, getParent)).toBe('sess-B');
		// 500 (miss) -> getParent -> 400 (miss) -> getParent -> 300 (hit).
		expect(getParent).toHaveBeenCalledTimes(2);
	});

	it('gives up after MAX_PID_WALK_HOPS hops', () => {
		const lookup = () => null;
		// An endlessly-deep chain that never resolves: 10 -> 9 -> 8 -> ...
		const getParent = vi.fn((pid: number) => (pid > 2 ? pid - 1 : null));

		expect(resolveSessionFromPidWalk(100, lookup, getParent)).toBeNull();
	...
	it('returns null when start pid is invalid', () => {
		const lookup = vi.fn(() => 'sess-X');
		const getParent = vi.fn(() => 200);

		expect(resolveSessionFromPidWalk(0, lookup, getParent)).toBeNull();
		expect(resolveSessionFromPidWalk(-1, lookup, getParent)).toBeNull();
		expect(resolveSessionFromPidWalk(1, lookup, getParent)).toBeNull();
		expect(lookup).not.toHaveBeenCalled();
	});

	it('stops walking when getParent reports a self-cycle or pid<=1', () => {
		const lookup = () => null;
		// Self-cycle: 50's parent is 50. Walk must terminate, not loop.
		const getParent = vi.fn((pid: number) => (pid === 50 ? 50 : null));

		expect(resolveSessionFromPidWalk(50, lookup, getParent)).toBeNull();
		expect(getParent).toHaveBeenCalledTimes(1);
	});

	it('stops walking when getParent returns pid 1 (init/launchd)', () => {
		const lookup = () => null;
		const getParent = vi.fn((pid: number) => (pid === 200 ? 1 : null));

		expect(resolveSessionFromPidWalk(200, lookup, getParent)).toBeNull();
		expect(getParent).toHaveBeenCalledTimes(1);
	});
});
